...
The first time PerovStats is running using 'cellpose' segmentation the model will have to be downloaded. A progress bar will appear in the output and it may take a few minutes to complete. This model can then be reused so you should not have to wait for it to download again.
